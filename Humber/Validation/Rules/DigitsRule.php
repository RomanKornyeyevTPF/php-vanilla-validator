...
class DigitsRule extends Rule
{
    ...
    function evaluate(array $request, string $field, array $options = null, string $message = null)
    {
        if (is_array($options) && ctype_digit($options[0]))
            return !(ctype_digit($request[$field]) && strlen($request[$field]) == $options[0]) ? (is_null($message) ? "'$field' no es un número válido o es más largo de lo permitido $options[0] " : $message) : null;
        else
